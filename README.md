...
scripts/        build.js, deploy-ecs.sh, smoke-test.sh, rollback-ecs.sh, tag-version.sh, promote-image.sh, mark-stable.sh
```
...
- **`release.yml`** (al mergear a `integracion`): calcula la versión semántica (Conventional Commits), construye la imagen, la escanea con Trivy (bloqueante) y la publica en el repositorio ECR del ambiente.
...
`scripts/deploy-ecs.sh`, `scripts/smoke-test.sh`, `scripts/rollback-ecs.sh`, `scripts/tag-version.sh` y `scripts/promote-image.sh` se pueden ejecutar localmente con credenciales AWS válidas — en la práctica solo el pipeline los invoca.
